...
const WeeklyWrapPage = async () => {
    ...
    const highestScore = reviewedTopics.length > 0 
        ? reviewedTopics.reduce((max, obj) => obj.score > max.score ? obj : max, { score: 0, note: { title: '' } })
        : { score: 0, note: { title: '' } };
    ...
    const unreviewedNotes = allNotesThisWeek.filter(note => note.exams.length === 0);
    const weakestNote = unreviewedNotes.length > 0 ? unreviewedNotes[0]?.title || "Unreviewd notes" : "None";
    ...
    const improvementCourses = Array.from(courseImprovements.values())
        .filter(course => course.thisWeekMax !== -Infinity && course.lastWeekMin !== Infinity);
    
    const mostImprovedCourse = improvementCourses.length > 0
        ? improvementCourses.reduce((max, course) => 
    ...
    const mostReviewedExams = reviewedTopics.filter(exam => exam.noteId === mostReviewedTopic.noteId);
    const mostReviewedPassedExams = mostReviewedExams.filter(exam => exam.score >= 70).length;
...
const WeeklyWrapPage = async () => {
  ...
  const prompt = `Generate a detailed weekly performance wrap-up for a user based on their study habits and exam results. The report should include key statistics, insights on strengths and weaknesses, and an AI-generated study suggestion. Additionally, assign the user a 'cat personality' based on their study behavior.
