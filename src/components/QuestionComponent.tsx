...
export default function QuestionComponent({question, choices, onAnswer}: QuestionComponentProps) {
    return (
                    ...
                    <div className="grid-cols-2 grid gap-5 w-full">
                        {choices.map((choice, index) => (
                            <button
                                ...
                                className="w-full bg-[#591DA9] p-4 text-xl rounded-xl text-white hover:bg-[#6929c4] transition-colors"
                            >
                                {choice}
                            </button>
                        ))}
